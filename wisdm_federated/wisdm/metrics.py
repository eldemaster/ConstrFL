"""
Advanced Metrics for Federated Learning - WISDM HAR
Metriche utili per analizzare qualità e fairness del FL per Human Activity Recognition
"""
...
def fit_metrics_aggregation(metrics: List[Tuple[int, Metrics]]) -> Metrics:
    ...
    """
    # Estrai valori
    train_accs = [m["train_acc"] for _, m in metrics if "train_acc" in m]
    ...
    weighted_acc = sum(m.get("train_acc", 0) * num for num, m in metrics) / sum(num_examples) if train_accs else 0
    weighted_loss = sum(m.get("train_loss", 0) * num for num, m in metrics) / sum(num_examples) if train_losses else 0
    
    # Statistiche di dispersione (quanto variano i client?)
    acc_std = np.std(train_accs) if train_accs else 0
    acc_min = np.min(train_accs) if train_accs else 0
    acc_max = np.max(train_accs) if train_accs else 0
...
def evaluate_metrics_aggregation(metrics: List[Tuple[int, Metrics]]) -> Metrics:
    ...
    """
    # Estrai valori
    eval_accs = [m["eval_acc"] for _, m in metrics if "eval_acc" in m]
    eval_losses = [m["eval_loss"] for _, m in metrics if "eval_loss" in m]
    num_examples = [num for num, _ in metrics]
    
    # Media pesata
    weighted_acc = sum(m.get("eval_acc", 0) * num for num, m in metrics) / sum(num_examples) if eval_accs else 0
    weighted_loss = sum(m.get("eval_loss", 0) * num for num, m in metrics) / sum(num_examples) if eval_losses else 0
    
    # Fairness metrics
    acc_std = np.std(eval_accs) if eval_accs else 0
    acc_min = np.min(eval_accs) if eval_accs else 0
    acc_max = np.max(eval_accs) if eval_accs else 0
...
def evaluate_model_detailed(model, x_test, y_test, num_classes=6):
    """
    Valutazione dettagliata del modello con metriche per classe
    Ottimizzato per WISDM HAR (6 classi di attività)
...
class MetricsLogger:
    ...
    🆕 Usa struttura organizzata: results/run_TIMESTAMP/
    """
    ...
    def __init__(self, experiment_name=None):
        ...
        if experiment_name:
            ...
            self.exp_name = f"wisdm_fl_experiment_{timestamp}"
    ...
    def _append_csv(self, round_data):
        ...
        # Se è il primo round con dati, riscriviamo il CSV con l'header completo
        if round_data['round'] == 1 or len(self.rounds_data) == 1:
            with open(self.csv_path, 'w', newline='') as f:
    ...
    def save_final_summary(self, additional_info=None):
        ...
        """
        summary_path = self.results_dir / f"{self.exp_name}_summary.txt"
        
        with open(summary_path, 'w') as f:
            f.write("=" * 70 + "\n")
            f.write(f"FEDERATED LEARNING EXPERIMENT SUMMARY - WISDM HAR\n")
            f.write("=" * 70 + "\n\n")
            ...
            if self.rounds_data:
                ...
                f.write("Activity Classes (WISDM):\n")
                f.write("  0: Walking\n")
                f.write("  1: Jogging\n")
                f.write("  2: Upstairs\n")
                f.write("  3: Downstairs\n")
                f.write("  4: Sitting\n")
                f.write("  5: Standing\n")
...
def print_metrics_summary(round_num, centralized_metrics, distributed_metrics, 
    ...
    """
    Stampa un summary leggibile delle metriche per WISDM
    """
    print(f"\n{'='*70}")
    print(f"ROUND {round_num} SUMMARY - WISDM HAR")
    print(f"{'='*70}")
