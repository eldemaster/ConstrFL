...
class LocalEarlyStopping:
    ...
    Args:
        ...
        min_epochs: Numero minimo di epochs prima di permettere early stopping (default: 0)
    ...
    def should_stop(self) -> bool:
        ...
        if self.current_epoch < self.min_epochs:
            if self.verbose and self.wait >= self.patience:
                print(f"    ⏸️  Would stop, but min_epochs not reached ({self.current_epoch}/{self.min_epochs})")
            return False
    ...
    def reset(self):
        """Reset stato per nuovo round di FL"""
        self.best_loss = float('inf')
        self.best_weights = None
        self.wait = 0
        self.stopped_epoch = 0
